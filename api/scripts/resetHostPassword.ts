...
async function main() {
  ...
  const password = process.env.SEED_HOST_PASSWORD ?? (await prompt("New password: ", true));
  ...
  if (!host) {
    console.error(`No host with email ${email} exists.`);
    await disconnectDb();
    process.exit(1);
  }

  host.passwordHash = await bcrypt.hash(password, 12);
  await host.save();

  console.log(`Password reset for ${host.email}.`);
  await disconnectDb();
}
