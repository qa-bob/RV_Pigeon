...
- [X] T038 [US1] Implement the Outdoorsy adapter's `postMessage` method in `agent/src/adapters/outdoorsy.ts`. **Revised 2026-09-06** with real selectors from DevTools-inspected HTML: a trip's Booking ID does not double as its message-thread id — postMessage opens the trip detail page first to find the real conversation link (a "message-circle icon" link, only discoverable there), then fills and submits the composer on that thread. Not yet live-tested (deliberately) — the first real invocation will send an actual message to a real guest, so this needs a careful, consciously-chosen verification step before `deliver` is trusted to run unattended.
